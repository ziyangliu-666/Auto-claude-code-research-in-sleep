...
allowed-tools: Bash(*), Read, Write, Edit, Grep, Glob, WebSearch, WebFetch, Agent, mcp__claude-review__review_start, mcp__claude-review__review_reply_start, mcp__claude-review__review_status
---
...
User input (PROBLEM + vague APPROACH)
  ...
  -> Phase 2 (Claude reviewer/Claude reviewer): Review for fidelity, specificity, contribution quality, and frontier leverage
...
Send the full proposal to Claude reviewer for an **elegance-first, frontier-aware, method-first** review. The reviewer should spend most of the critique budget on the method itself, not on expanding the experiment menu.
...
Send the revised proposal back to Claude reviewer in the **same thread**:
...
[Full verbatim response from Claude reviewer]
...
- **Save `threadId` from Phase 2** and use `mcp__claude-review__review_reply_start` for later rounds.
