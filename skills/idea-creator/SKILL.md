...
allowed-tools: Bash(*), Read, Write, Grep, Glob, WebSearch, WebFetch, Agent, mcp__claude-review__review_start, mcp__claude-review__review_reply_start, mcp__claude-review__review_status
---
...
Use the external LLM via claude-review MCP for divergent thinking:
...
1. **Novelty check**: Use the `/novelty-check` workflow (multi-source search + Claude reviewer cross-verification) for each idea

2. **Critical review**: Use Claude reviewer via `mcp__claude-review__review_reply_start` (same thread):
   ```
...
3. **Combine rankings**: Merge your assessment with Claude reviewer's ranking. Select top 2-3 ideas for pilot experiments.
