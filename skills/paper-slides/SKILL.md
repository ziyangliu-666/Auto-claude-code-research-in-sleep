...
### Phase 5: claude-review MCP Review

Send the slide outline + selected LaTeX frames to Claude reviewer:
...
> ⚠️ If `mcp__claude-review__review_start` is not available (no OpenAI API key), skip external review and proceed to Phase 6. Note the skip in `SLIDES_STATE.json`.
...
- Files:
  ...
  ├── SLIDES_REVIEW.md      # Claude reviewer review feedback
