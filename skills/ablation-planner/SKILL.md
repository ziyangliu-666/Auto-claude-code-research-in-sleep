...
description: "Use when main results pass result-to-claim (claim_supported=yes or partial) and ablation studies are needed for paper submission. Claude reviewer designs ablations from a reviewer's perspective, CC reviews feasibility and implements."
...
Systematically design ablation studies that answer the questions reviewers will ask. Claude reviewer leads the design (reviewer perspective), CC reviews feasibility and implements.
...
### Step 2: Claude Reviewer Designs Ablations
...
Normalize Claude reviewer response into structured format:
...
- Cuts: if budget is tight, propose removing lower-priority ablations and ask Claude reviewer to confirm
...
- **Claude reviewer leads the design. CC does not pre-filter or bias the ablation list** before Claude reviewer sees it. Claude reviewer thinks like a reviewer; CC thinks like an engineer.
...
- If total compute exceeds budget, CC proposes cuts and asks Claude reviewer to re-prioritize — don't silently drop ablations.
